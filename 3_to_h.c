...
void show_table_from(int v[MAXCITY][MAXCONN], int org, struct train trains[]);
void show_table_to(int v[MAXCITY][MAXCONN], int org, struct train rtrains[]);
...
int main(void)
{
    ...
    while (1) {
        ...
        make_table(to_hakodate, city_id("Hakodate"), rtrains); /* to_h の作成 */
        make_table(to_tokyo, city_id("Tokyo"), rtrains); /* to_t の作成 */

        printf("from Hakodate\n");
        show_table_from(from_hakodate, city_id("Hakodate"), trains); /* fr_h の表示 */

        printf("from Tokyo\n");
        show_table_from(from_tokyo, city_id("Tokyo"), trains); /* fr_t の表示 */

        printf("to Hakodate\n");
        show_table_to(to_hakodate, city_id("Hakodate"), rtrains); /* to_h の表示 */

        printf("to Tokyo\n");
        show_table_to(to_tokyo, city_id("Tokyo"), rtrains); /* to_t の表示 */
    }
}
...
void show_table_to(int v[MAXCITY][MAXCONN], int org, struct train rtrains[])
{
    ...
    for (ti = 0; ti < nconn ; ti++) {
        printf("| %d:%02d ", ((BIAS - rtrains[ti].arv)/60), ((BIAS - rtrains[ti].arv)%60));
        for (i = 0; i < ncity ; i++) {
            if (v[i][ti+1] == INFINITE) {
                printf("| INF ");
            } else {
